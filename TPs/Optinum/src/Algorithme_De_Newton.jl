...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    nb_iters = 0
    for k in 1:max_iter
        d_k = (hessf(x_k))\(-gradf(x_k))
        x_k = x_k + d_k
        if norm(gradf(x_k + d_k),2) <= max(norm(gradf(x0),2)*Tol_rel,Tol_abs)
            flag = 0
            break
        elseif norm(d_k,2) <= max(norm(x_k,2)*Tol_rel,Tol_abs)
            flag = 1
            break
        elseif abs(f(x_k+d_k)-f(x_k)) <= max(abs(f(x_k))*Tol_rel,Tol_abs)
            flag = 2
            break
        end
        nb_iters += 1

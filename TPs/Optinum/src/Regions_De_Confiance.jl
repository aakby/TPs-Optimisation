...
function Regions_De_Confiance(algo,f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    for k in 1:max_iter
        ...
        # Calculer approximativement s_k solution du sous-problème.
        if algo == "cauchy"
            s_k, e_k = Pas_De_Cauchy(gradf(x_k),hessf(x_k),delta_k)
        elseif algo =="gct"
            s_k = Gradient_Conjugue_Tronque(gradf(x_k),hessf(x_k),[delta_k,max_iter,Tol_abs])
        end
        
         #Calculer le modèle
        function m(s)
            return f(s) + (gradf(x_k))'*(s-x_k) + 1/2*(s-x_k)'*hessf(x_k)*(s-x_k)
        end
        
        #Calculer rho_k
        rho_k = (f(x_k) - f(x_k+s_k))/(m(x_k) - m(x_k+s_k))
        
            
        # Mettre à jour l'itéré courant
        if rho_k >= eta1
            x_k = x_k + s_k
        end
        
        # Mettre à jour la région de confiance
        if rho_k > eta2
    ...
    return x_min, f_min, flag, nb_iters
end
